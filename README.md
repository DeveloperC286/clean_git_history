...
jobs:
  clean:
    ...
    container:
      image: ghcr.io/developerc286/clean_git_history:v1.2.2
    steps:
      - name: Checkout code.
...
<!-- x-release-please-start-version -->
```sh
version="v1.2.2" && wget -O - "https://github.com/DeveloperC286/clean_git_history/releases/download/${version}/x86_64-unknown-linux-musl.tar.gz" | tar xz --directory "/usr/bin/"
```
<!-- x-release-please-end -->
...
<!-- x-release-please-start-version -->
```sh
cargo install clean_git_history --version "1.2.2"
```
<!-- x-release-please-end -->
...
<!-- x-release-please-start-version -->
```sh
docker run --rm -v $(pwd):/workspace -w /workspace ghcr.io/developerc286/clean_git_history:v1.2.2 origin/HEAD
```
<!-- x-release-please-end -->
